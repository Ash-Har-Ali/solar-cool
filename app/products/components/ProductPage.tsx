...
const ProductPage: React.FC<ProductPageProps> = ({
  ...
  const sortedAndFilteredProducts = useMemo(() => {
    return filteredProducts
      .filter((product) =>
        filters.filterBLDC === null ? true : product.bldc === filters.filterBLDC
      )
      .sort((a, b) =>
    ...
    <div className="bg-[#e7e7e7]">
      <div className="relative mb-6">
        <Image
          ...
          alt={`${title} Banner`}
          className="hidden sm:block object-cover w-full h-[250px] sm:h-[350px] md:h-[500px] lg:h-[730px] 2xl:h-[900px] mt-7"
          width={1920}
          height={1080}
          priority
        />
        <Image
          ...
          alt={`${title} Banner`}
          className="block sm:hidden object-cover w-full h-[600px] mt-7"
          width={768}
          height={500}
          priority
        />
        <div className="container mx-auto px-6 sm:px-12 py-8 absolute inset-0 flex items-start mt-20 md:mt-0 md:mb-10 sm:items-center sm:justify-start text-white text-center text-4xl md:text-6xl font-bold font-['Montserrat']">
          <div className="w-full sm:w-auto sm:relative sm:top-auto items-start">
            {title}
          </div>
        </div>
      </div>
      <div className="container mx-auto px-4 sm:px-12 py-8">
        <div className="container mx-auto  flex flex-wrap md:justify-center justify-around md:space-x-4 mb-12 gap-4">
          <button
            onClick={() => setFilters({ ...filters, filterBLDC: null })}
            className="px-4 py-2 bg-white text-[#036d39] font-medium rounded-md hover:bg-[#036d39] hover:text-white transition duration-300"
          >
            All Products
          </button>
          <button
            onClick={() => setFilters({ ...filters, sortOrder: "price-asc" })}
            className="px-4 py-2 bg-white text-[#036d39] font-medium rounded-md hover:bg-[#036d39] hover:text-white transition duration-300"
          >
            Lowest Price
          </button>
          <button
            onClick={() => setFilters({ ...filters, sortOrder: "price-desc" })}
            className="px-4 py-2 bg-white text-[#036d39] font-medium rounded-md hover:bg-[#036d39] hover:text-white transition duration-300"
          >
            Highest Price
          </button>
          <button
            onClick={() => setFilters({ ...filters, filterBLDC: true })}
            className="px-4 py-2 bg-white text-[#036d39] font-medium rounded-md hover:bg-[#036d39] hover:text-white transition duration-300"
          >
            BLDC Only
          </button>
