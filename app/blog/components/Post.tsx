      ...
      <main className="container mx-auto prose prose-xl px-6 py-16 bg-white rounded-2xl shadow-2xl mb-12 transition-all duration-300">
        ...
        {post?.mainImage && (
          <div className="mb-12 justify-items-center">
            <Image
              src={
                builder.image(post.mainImage).width(900).height(600).url() || ""
              }
              alt={post?.mainImage?.alt || "Main image"}
              width={900}
              height={600}
              className="rounded-2xl shadow-xl hover:shadow-2xl transition-transform transform hover:scale-105 duration-300"
              priority
            />
          </div>
        )}
        ...
        <header className="text-center mb-8 animate-fadeInUp">
          ...
          <p className="mt-4 text-lg text-gray-600 italic">{post.description}</p>
        </header>

        {/* Meta Data Section */}
        <div className="flex flex-wrap justify-center space-x-6 text-sm text-gray-700 mb-12">
